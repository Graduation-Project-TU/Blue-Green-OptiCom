...
// =====================================================
// RMT + Manchester SECTION
// =====================================================

// -----------------------------
// Hardware configuration
// -----------------------------
static const gpio_num_t RMT_RX_PIN = GPIO_NUM_16;
...
static bool IRAM_ATTR onRxDone(rmt_channel_handle_t channel,
...
static bool receiveSymbols(std::vector<rmt_symbol_word_t> &outSymbols) {
  ...
  if (!rxInProgress) {
    ...
    cfg.signal_range_max_ns = 60000000;

    esp_err_t err = rmt_receive(rxChannel,
                                rxRawBuffer,
                                sizeof(rxRawBuffer),
                                &cfg);
...
static int durationToHalfBits(uint16_t durationUs) {
  ...
  int n = (durationUs + (MANCHESTER_HALF_BIT_US / 2))
...
static bool decodeManchesterSymbols(
  ...
  auto decodeFromPhase = [&](size_t phase, String &outText) -> bool {
    ...
    auto extractCsvLine = [&](const String &txt,
                              String &lineOut) -> bool {
  ...
  if (decodeFromPhase(0, candidate) ||
      decodeFromPhase(1, candidate)) {
...
static bool parseAndPrint(const String &line) {
  ...
  if (!parseFloatField(tempStr, temp, tempIsNan) ||
      !parseFloatField(distStr, dist, distIsNan)) {
...
static bool IRAM_ATTR onRxDone(
...
static bool parseFloatField(const String &s,
                            float &outValue,
                            bool &isNan) {
